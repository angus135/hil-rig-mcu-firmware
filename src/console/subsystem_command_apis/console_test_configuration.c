...
static void CONSOLE_TestConfiguration_PrintUsage( void )
{
    ...
    CONSOLE_Printf( "  test_config pwm_generation <1-2> <off|3v3|5v|12v|24v> <arr> <ccr> <psc>\r\n" );
    CONSOLE_Printf( "  test_config can <1-2> <off|bitrate> [filter_bank filter_id filter_mask]\r\n" );
    CONSOLE_Printf( "  test_config spi <1-2> off\r\n" );
    CONSOLE_Printf( "  test_config spi <1-2> <master|slave> <8|16> <mode0|mode1|mode2|mode3> default_cs\r\n" );
...
{
    if ( strcmp( token, "off" ) == 0 ) { *voltage = EXEC_PWM_GEN_VOLTAGE_DISABLED; }
    else if ( strcmp( token, "3v3" ) == 0 ) { *voltage = EXEC_PWM_GEN_VOLTAGE_3V3; }
    else if ( strcmp( token, "5v" ) == 0 ) { *voltage = EXEC_PWM_GEN_VOLTAGE_5V; }
    else if ( strcmp( token, "12v" ) == 0 ) { *voltage = EXEC_PWM_GEN_VOLTAGE_12V; }
    else if ( strcmp( token, "24v" ) == 0 ) { *voltage = EXEC_PWM_GEN_VOLTAGE_24V; }
    else { return false; }
    return true;
}
...
static bool CONSOLE_TestConfiguration_ParseDigitalInputMode(
    const char* token, ExecDigitalInputMode_T* mode )
{
...
static bool CONSOLE_TestConfiguration_ParsePwmCaptureMode(
    const char* token, ExecPwmCaptureConfig_T* configuration )
{
...
static void CONSOLE_TestConfiguration_PrintStatus( void )
{
    ...
    {
        const ExecDigitalOutputChannelConfig_T* item = &configuration.digital_outputs.channels[channel];
    ...
    for ( uint32_t channel = 0U; channel < EXEC_PWM_GEN_CHANNEL_COUNT; channel++ )
    {
        ...
        CONSOLE_Printf( " ch%u=%s(v=%u,arr=%u,ccr=%u,psc=%u)",
                        ( unsigned int )( channel + 1U ), item->is_enabled ? "on" : "off",
                        ( unsigned int )item->voltage_level, ( unsigned int )item->initial_arr,
                        ( unsigned int )item->initial_ccr, ( unsigned int )item->initial_psc );
    }
    CONSOLE_Printf( "\r\nCAN:" );
    ...
    for ( uint32_t channel = 0U; channel < EXEC_UART_CHANNEL_COUNT; channel++ )
    {
        ...
        CONSOLE_Printf( " ch%u=%s(mode=%u,%lu bit/s,rx=%u,tx=%u)",
                        ( unsigned int )( channel + 1U ), item->is_enabled ? "on" : "off",
                        ( unsigned int )item->interface_mode, ( unsigned long )item->baud_rate,
                        ( unsigned int )item->rx_enabled, ( unsigned int )item->tx_enabled );
    }
    CONSOLE_Printf( "\r\nI2C: forced disabled\r\n" );
}
...
void CONSOLE_TestConfiguration_Command( uint16_t argc, char* argv[] )
{
    ...
    else if ( argc == 3U && strcmp( argv[1], "analogue_output" ) == 0 )
    {
        ...
        else { CONSOLE_TestConfiguration_PrintUsage(); return; }
    }
    else if ( argc == 5U && strcmp( argv[1], "digital_output" ) == 0 )
    {
        ...
        { CONSOLE_TestConfiguration_PrintUsage(); return; }
        ExecDigitalOutputChannelConfig_T* item = &configuration.digital_outputs.channels[channel - 1U];
    ...
    else if ( argc == 7U && strcmp( argv[1], "pwm_generation" ) == 0 )
    {
        ...
        if ( !CONSOLE_TestConfiguration_ParseU32( argv[2], &channel ) || channel < 1U
             ...
             || ccr > ( arr + 1U ) )
        { CONSOLE_TestConfiguration_PrintUsage(); return; }
        configuration.pwm_generation_channels[channel - 1U] = ( ExecPwmGenConfig_T ){
            .is_enabled = voltage != EXEC_PWM_GEN_VOLTAGE_DISABLED, .voltage_level = voltage,
            .initial_arr = ( uint16_t )arr, .initial_ccr = ( uint16_t )ccr,
    ...
    else if ( ( argc == 4U || argc == 7U ) && strcmp( argv[1], "can" ) == 0 )
    {
        uint32_t channel = 0U;
        if ( !CONSOLE_TestConfiguration_ParseU32( argv[2], &channel ) || channel < 1U
             || channel > EXEC_CAN_CHANNEL_COUNT ) { CONSOLE_TestConfiguration_PrintUsage(); return; }
        EXEC_CAN_Config_T* item = &configuration.can_channels[channel - 1U];
        if ( argc == 4U && strcmp( argv[3], "off" ) == 0 ) { *item = ( EXEC_CAN_Config_T ){ 0 }; }
        else
        {
            ...
            { CONSOLE_TestConfiguration_PrintUsage(); return; }
            *item = ( EXEC_CAN_Config_T ){ true, bitrate, ( uint16_t )bank,
    ...
    else if ( ( argc == 4U || argc == 7U ) && strcmp( argv[1], "spi" ) == 0 )
    {
        uint32_t channel = 0U;
        if ( !CONSOLE_TestConfiguration_ParseU32( argv[2], &channel ) || channel < 1U
             || channel > TEST_CONFIGURATION_SPI_CHANNEL_COUNT ) { CONSOLE_TestConfiguration_PrintUsage(); return; }
        ExecSPIConfig_T* item = &configuration.spi_channels[channel - 1U];
        if ( argc == 4U && strcmp( argv[3], "off" ) == 0 ) { *item = ( ExecSPIConfig_T ){ 0 }; }
        else if ( argc == 7U )
        {
            const bool master = strcmp( argv[3], "master" ) == 0;
            const bool slave = strcmp( argv[3], "slave" ) == 0;
            if ( ( !master && !slave ) || ( strcmp( argv[4], "8" ) != 0 && strcmp( argv[4], "16" ) != 0 )
                 || strncmp( argv[5], "mode", 4U ) != 0 || argv[5][4] < '0' || argv[5][4] > '3' || argv[5][5] != '\0'
                 || strcmp( argv[6], "default_cs" ) != 0 ) { CONSOLE_TestConfiguration_PrintUsage(); return; }
            const uint8_t mode = ( uint8_t )( argv[5][4] - '0' );
            *item = ( ExecSPIConfig_T ){ .is_enabled = true, .hardware = {
                .spi_mode = master ? SPI_MASTER_MODE : SPI_SLAVE_MODE,
                .data_size = strcmp( argv[4], "16" ) == 0 ? SPI_SIZE_16_BIT : SPI_SIZE_8_BIT,
                .first_bit = SPI_FIRST_MSB, .baud_rate = SPI_BAUD_1M406BIT,
    ...
    else if ( ( argc == 4U || argc == 6U ) && strcmp( argv[1], "uart" ) == 0 )
    {
        uint32_t channel = 0U;
        if ( !CONSOLE_TestConfiguration_ParseU32( argv[2], &channel ) || channel < 1U
             || channel > EXEC_UART_CHANNEL_COUNT ) { CONSOLE_TestConfiguration_PrintUsage(); return; }
        ExecUartConfig_T* item = &configuration.uart_channels[channel - 1U];
        if ( argc == 4U && strcmp( argv[3], "off" ) == 0 ) { *item = ( ExecUartConfig_T ){ 0 }; }
        else
        {
            uint32_t baud = 0U;
            ExecUartInterfaceMode_T mode = EXEC_UART_MODE_DISABLED;
            if ( argc != 6U || !CONSOLE_TestConfiguration_ParseU32( argv[4], &baud ) || baud == 0U
                 || ( strcmp( argv[3], "3v3" ) != 0 && strcmp( argv[3], "5v" ) != 0 && strcmp( argv[3], "rs232" ) != 0 )
                 || ( strcmp( argv[5], "rx" ) != 0 && strcmp( argv[5], "tx" ) != 0 && strcmp( argv[5], "both" ) != 0 ) )
            { CONSOLE_TestConfiguration_PrintUsage(); return; }
            mode = strcmp( argv[3], "3v3" ) == 0 ? EXEC_UART_MODE_TTL_3V3
                 : strcmp( argv[3], "5v" ) == 0 ? EXEC_UART_MODE_TTL_5V0 : EXEC_UART_MODE_RS232;
            ...
            *item = ( ExecUartConfig_T ){ .interface_mode = mode, .baud_rate = baud,
                .word_length = HW_UART_WORD_LENGTH_8_BITS, .stop_bits = HW_UART_STOP_BITS_1,
                .parity = HW_UART_PARITY_NONE, .rx_enabled = strcmp( argv[5], "tx" ) != 0,
                .tx_enabled = strcmp( argv[5], "rx" ) != 0, .is_enabled = true };
        }
    }
    else
    {
