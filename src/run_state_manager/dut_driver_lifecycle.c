...
static bool DUT_DRIVER_LIFECYCLE_StopAttempt( bool force_abort, bool* busy )
{
    ...
    *busy                                  = false;
    ...
    for ( uint32_t channel = EXEC_UART_CHANNEL_COUNT; channel > 0U; channel-- )
    {
        ...
        const ExecUartTxStatus_T tx_status = EXEC_UART_Get_Tx_Status( ( ExecUartChannel_T )index );
        if ( !force_abort
             && ( tx_status == EXEC_UART_TX_STATUS_FAULTED
                  || EXEC_UART_Is_Rx_Faulted( ( ExecUartChannel_T )index ) ) )
        {
            failed = true;
        }
