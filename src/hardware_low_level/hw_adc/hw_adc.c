...
void HW_ADC_Start_DMA_Measurements( void )
{
    ...
    HAL_ADC_Start_DMA( HW_ADC_ADC_PERIPHERAL, ( uint32_t * )adc_dma_buf, ADC_DMA_LEN*sizeof(ADCMeasurement_T)/sizeof(uint16_t) );
#endif
}
...
void HW_ADC_Read_DMA_Measurements( ADCMeasurement_T* measurements, uint32_t number )
{
    ...
    uint32_t current_index =
        (ADC_DMA_LEN - LL_DMA_GetDataLength( HW_ADC_DMA_CHANNEL, HW_ADC_DMA_STREAM )) >> 1; // Dividing by 2 because of 2 ADC measurements
