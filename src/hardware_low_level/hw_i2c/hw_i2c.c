...
static inline bool hw_i2c_config_is_valid( const HW_I2C_ChannelConfig_T* config );
...
static inline void hw_i2c_set_speed_and_address( I2C_TypeDef* i2c_instance,
												 HW_I2C_Speed_T speed,
...
static void hw_i2c_configure_dma_stream( DMA_Stream_TypeDef* stream,
...
static inline void hw_i2c_service_event_external( HW_I2C_Channel_T channel, I2C_TypeDef* i2c_instance );
...
static inline void hw_i2c_service_event_external( HW_I2C_Channel_T channel, I2C_TypeDef* i2c_instance )
{
	...
	if ( ( sr1 & I2C_SR1_SB ) != 0U )
	{
		...
		i2c_instance->DR = ( uint32_t )( ( uint8_t )( ( state->target_address_7bit << 1U ) | direction_bit ) );
		return;
	}
		 ...
		 state->transfer_kind == HW_I2C_TRANSFER_KIND_SLAVE_TX )
	{
		...
		if ( ( state->transfer_kind == HW_I2C_TRANSFER_KIND_MASTER_TX ) && ( state->tx_remaining == 0U ) &&
			 ( ( sr1 & I2C_SR1_BTF ) != 0U ) )
		{
			hw_i2c_finish_transfer( channel, i2c_instance );
			return;
...
static inline void hw_i2c_service_event_fmpi2c1( HW_I2C_ChannelState_T* state )
{
	...
	if ( ( isr & ( FMPI2C_ISR_BERR | FMPI2C_ISR_ARLO | FMPI2C_ISR_OVR | FMPI2C_ISR_TIMEOUT ) ) != 0U )
	{
...
{
	if ( !hw_i2c_is_external_channel( channel ) || !hw_i2c_config_is_valid( config ) )
	{
		return HW_I2C_STATUS_INVALID_PARAM;
	}
...
HW_I2C_Status_T HW_I2C_Trigger_Master_Transmit( HW_I2C_Channel_T channel, uint16_t device_address_7bit )
{
	HW_I2C_ChannelState_T* state = NULL;
	HW_I2C_Status_T status        = hw_i2c_validate_active_channel( channel, &state );
	if ( status != HW_I2C_STATUS_OK )
	...
	if ( hw_i2c_is_external_channel( channel ) )
	{
		...
		if ( use_dma )
		{
			...
			hw_i2c_configure_dma_stream( tx_stream,
										 hw_i2c_channel_to_dma_channel_bits( channel ),
										 true,
...
HW_I2C_Status_T HW_I2C_Trigger_Master_Receive( HW_I2C_Channel_T channel, uint16_t device_address_7bit,
											   uint16_t expected_length )
{
	...
	if ( hw_i2c_is_external_channel( channel ) )
	{
		...
		if ( use_dma )
		{
			...
			hw_i2c_configure_dma_stream( rx_stream,
										 hw_i2c_channel_to_dma_channel_bits( channel ),
										 false,
										 ( uint32_t )&i2c_instance->DR,
										 ( uint32_t )state->dma_rx_linear_buffer,
										 expected_length );
		}
