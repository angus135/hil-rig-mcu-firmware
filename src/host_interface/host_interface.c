...
void HOST_INTERFACE_Task( void* task_parameters )
{
    ...
    while ( true )
    {
            ...
            while ( HW_USB_Get_Receive_Stream_Used_Bytes() > 0 )
        {
            ...
            uint32_t bytes_read = HW_USB_Receive( usb_receive_buffer, usb_recieve_size )
            ...
            {
                t_status = HIL_TRANSPORT_Receive_Bytes( &t_context, &( usb_receive_buffer[offset] ),
            ...
            a_status = HIL_TRANSPORT_Read_Application_Data( &t_context, &a_recieve_byte_span,
                                                            a_byte_span_capacity, &a_used_capacity )

                if ( a_status == HIL_APPLICATION_STATUS_OK )
            {
                // =======------- CHECK DECODED DATA SIZE
                size_t required_decode_capacity = 0;
                a_status                        = HIL_APPLICATION_Decode_Storage_Size(
                    a_context, &a_recieve_byte_span, a_used_capacity,
                    &required_decode_capacity ) if ( a_status != HIL_APPLICATION_STATUS_OK
                                                     || required_decode_capacity > a_data_capacity )
                {
                    Error_Handler();
                    break;
                }
                // =======------- DECODE APPLICATION MESSAGE
                a_status = HIL_APPLICATION_Decode_Message(
                    &a_context, a_receive_byte_span, a_used_byte_span_capacity, &a_receive_message,
                    a_receive_data, a_data_capacity, &a_used_data_capacity );
            ...
            t_status = HIL_TRANSPORT_Read_Event( &t_context, &t_event );
            if ( t_status != HIL_TRANSPORT_STATUS_OK )
            {
                Error_Handler();
                break;
        ...
        if ( output_pending == 0 )
        {
            ...
            t_status = HIL_TRANSPORT_Peek_Output(
                &t_context, &t_output_buffer, t_output_buffer_size,
                &t_used_output_buffer_size ) if ( t_status == HIL_TRANSPORT_STATUS_OK )
            {
